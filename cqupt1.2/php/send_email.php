...
$code = makecode(8);
mysqli_query($a,"UPDATE USERINFO SET verification_code='".$code."' WHERE ID='".$id."'");
mysqli_query($a,"UPDATE USERINFO SET verification_time='".time()."' WHERE ID='".$id."'");
$return_words = "亲爱的：".$id.",您的验证码是：".$code."。请进入下方链接重置您的密码，谢谢您的配合！（验证码10分钟内有效）";
$return_words .= "<br>"."<a href='http://localhost/photosbasement2/cqupt1.2/password.html'>http://localhost/photosbasement2/cqupt1.2/password.html</a>";
$mail->Body = $return_words;
// 为该邮件添加附件
//$mail->addAttachment('./example.pdf');
// 发送邮件 返回状态
$status = $mail->send();
if($status) echo json_encode(["status" => true]);
