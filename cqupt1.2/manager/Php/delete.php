...
if ($path0 == "http://127.0.0.1/photosbasement2/cqupt0.9/manager/img/test.png" or $path0 == "http://127.0.0.1/photosbasement2/cqupt0.9/manager/examine.html") {     //待修改
    ...
    if (isset($words)) {      //有反馈
        ...
        $label = $arr['label'];
        $ID = $arr['ID'];
        preg_match('#/(?<=tempfile/).+#', $matches[0], $matches2);
        $original_path = "../tempfile" . $matches2[0];
        $new_path = "../../view_delete" . $matches2[0];
        copy($original_path, $new_path);
        $words = $words . "（抱歉！您上传的标签为：" . $label . "的图片已被图库管理人员删除！）";
        $ensure = mysqli_query($a, "SELECT feedback,view_delete_path FROM USERINFO WHERE ID='" . $ID . "'");
        $arr = $ensure->fetch_assoc();
        if ($arr['feedback'] != NULL) {   //反馈非空
            $words = $arr['feedback'] . ',' . $words;
            $view = $arr['view_delete_path'] . ',' . "./view_delete" . $matches2[0];
        }
