...
if ($path0 == "http://127.0.0.1/photosbasement2/cqupt0.9/manager/img/test.png") {
    ...
    preg_match('#/(?<=file/).+#', $matches[0], $matches2);
    $original_path = "../../file" . $matches2[0];
    $new_path = "../../view_delete" . $matches2[0];
    copy($original_path, $new_path);
    $words .=  "（抱歉！您上传的标签为：" . $label . "的图片已被图库管理人员删除！）";
    $res = mysqli_query($a, "SELECT feedback AND uploading AND view_delete_path FROM USERINFO WHERE id='" . $id . "'");
    $content = $res->fetch_assoc();
    if ($content['feedback'] != NULL) {   //反馈非空
        $words = $content['feedback'] . ',' . $words;
        $view = $content['view_delete_path'] . ',' . "./file" . $matches2[0];
    }
    mysqli_query($a, "UPDATE USERINFO SET feedback='" . $words . "',view_delete_path = '".$view."' WHERE ID = '" . $id . "'");
